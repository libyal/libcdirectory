...
static void *(*cdirectory_test_real_memcpy)(void *, const void *, size_t) = NULL;
...
int cdirectory_test_memcpy_attempts_before_fail                           = -1;
...
void *memcpy(
       ...
       size_t size )
{
	if( cdirectory_test_real_memcpy == NULL )
	{
		cdirectory_test_real_memcpy = dlsym(
		                               RTLD_NEXT,
		                               "memcpy" );
	}
	if( cdirectory_test_memcpy_attempts_before_fail == 0 )
	{
		cdirectory_test_memcpy_attempts_before_fail = -1;

		return( NULL );
	}
	else if( cdirectory_test_memcpy_attempts_before_fail > 0 )
	{
		cdirectory_test_memcpy_attempts_before_fail--;
	}
	destination = cdirectory_test_real_memcpy(
	               destination,
	               source,
	               size );
