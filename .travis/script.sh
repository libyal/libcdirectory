...
then
	docker run -t -v "${PWD}:/libcdirectory" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libcdirectory && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libcdirectory;
