# Script that builds libcdirectory
#
# Version: 20260608
...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libcdirectory.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libcdirectory.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libcdirectory.sln"
}
